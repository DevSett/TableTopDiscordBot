...
public abstract class Command
{
    ...
    private final static String BOT_PERM = "%s Мне нужно разрешение %s в этом %s!";
    private final static String USER_PERM = "%s Вы должны иметь разрешение %s в этом %s чтобы использовать его!";
    ...
    public final void run(CommandEvent event)
    {
        ...
        {
            terminate(event, "Команда не может быть использована в этом канале!");
            return;
        }
        ...
        if(requiredRole!=null)
            ...
            {
                terminate(event, event.getClient().getError()+" У вас должна быть роль `"+requiredRole+"` для использования команды!");
                return;
            }
            ...
            for(Permission p: botPermissions)
            {
                if(p.isChannel())
                {
                    if(p.name().startsWith("VOICE"))
                    {
                        ...
                        if(vc==null)
                        {
                            terminate(event, event.getClient().getError()+" Вы должны быть в голосовом канале, чтобы использовать команду!");
                            return;
                        }
        ...
        {
            terminate(event, event.getClient().getError()+" Эта команда не может быть использована в ЛС");
            return;
        }
        ...
        try {
            ...
            event.reply(discordException.getMessage());
        } catch(Throwable t) {
    ...
    public String getCooldownError(CommandEvent event, int remaining)
    {
        if(remaining<=0)
            return null;
        String front = event.getClient().getWarning()+" Команда станет доступна через "+remaining+" секунд";
