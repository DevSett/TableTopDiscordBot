...
public class CommandClientImpl implements CommandClient, EventListener
{
                             ...
                             int linkedCacheSize, AnnotatedModuleCompiler compiler, GuildSettingsManager manager)
    {
        ...
        this.helpConsumer = helpConsumer==null ? (event) -> {
                StringBuilder builder = new StringBuilder("**"+event.getSelfUser().getName()+"** команды:\n");
                Category category = null;
                for(Command command : commands)
                {
                    if(!command.isHidden() && (!command.isOwnerCommand() || event.isOwner())
                            && (command.getRequiredRole() == null
                            || event.getMember().getRoles().stream().anyMatch(role -> role.getName().equals(command.getRequiredRole()))))
                    {
                        if(!Objects.equals(category, command.getCategory()))
                        {
                            category = command.getCategory();
                            builder.append("\n\n  __").append(category==null ? "Нет Категории" : category.getName()).append("__:\n");
                        }
                ...
                if(owner!=null)
                {
                    builder.append("\n\nЗа дополнительной помощью обращайтесь **").append(owner.getName()).append("**#").append(owner.getDiscriminator());
                    if(serverInvite!=null)
                        builder.append(" или присоединиться ").append(serverInvite);
                }
                event.replyInDm(builder.toString(), unused ->
                {
                    if(event.isFromType(ChannelType.TEXT))
                        event.reactSuccess();
                }, t -> event.replyWarning("Хелп не может быть отправлен, потому что вы блокируете ЛС с ботом."));
        } : helpConsumer;
